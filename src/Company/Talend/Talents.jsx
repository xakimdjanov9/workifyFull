...
const Talents = () => {
  ...
  useEffect(() => {
    ...
    if (occupation) result = result.filter((t) => t.occupation?.toLowerCase().includes(occupation.toLowerCase()));
    if (speciality) result = result.filter((t) => t.specialty?.toLowerCase().includes(speciality.toLowerCase()));
    if (selectedSpecs.length > 0) result = result.filter((t) => selectedSpecs.includes(t.specialty));

    if (selectedLangs.length > 0) {
      result = result.filter((talent) => {
        try {
          const tLangs = typeof talent.language === "string" ? JSON.parse(talent.language) : talent.language || [];
          return selectedLangs.some((sl) => tLangs.some((tl) => tl.language.toLowerCase() === sl.toLowerCase()));
        } catch { return false; }
      });
    }
    ...
    if (activeSkills.length > 0) {
      result = result.filter((talent) => {
        try {
          const talentSkills = typeof talent.skils === "string" ? JSON.parse(talent.skils) : talent.skils || [];
          return activeSkills.every((fRow) =>
            talentSkills.some((ts) => {
              const skillMatch = ts.skill?.toLowerCase().includes(fRow.skill.toLowerCase());
              // Agar experience bo'sh bo'lsa, faqat skillni tekshir. Bo'sh bo'lmasa, >= mantiqini qo'lla.
              const expMatch = fRow.experience === "" || Number(ts.experience_years) >= Number(fRow.experience);
    ...
    if (workplaceType) result = result.filter((t) => t.workplace_type === workplaceType);
    if (salaryRange.min) result = result.filter((t) => (t.minimum_salary || 0) >= Number(salaryRange.min));
    if (salaryRange.max) result = result.filter((t) => (t.minimum_salary || 0) <= Number(salaryRange.max));
    if (location.country) result = result.filter((t) => t.country?.toLowerCase().includes(location.country.toLowerCase()));
    if (location.city) result = result.filter((t) => t.city?.toLowerCase().includes(location.city.toLowerCase()));

    setFilteredTalents(result);
  }, [occupation, speciality, skillsRows, selectedSpecs, selectedLangs, workType, workplaceType, salaryRange, location, allTalents]);
      ...
      <div className="max-w-[1240px] mx-auto">
          ...
          <div className="bg-white rounded-[20px] shadow-sm border border-gray-100 p-2 flex items-center justify-between gap-2">
            <div className="flex flex-1 gap-1 md:gap-2 overflow-x-auto no-scrollbar pb-1 md:pb-0">
              {["Specialty", "Skills and expertice", "Preferences"].map((tab) => (
                ...
                >
                  {activeTab === tab && (
                    <motion.div layoutId="tabBg" className="absolute inset-0 bg-[#F8F9FA] shadow-sm rounded-[15px] -z-10" />
                  )}
                  {tab}
                </button>
              ))}
                    ...
                    {[...new Set(allTalents.map((t) => t.specialty).filter(Boolean))].map((spec, i) => (
                      <label key={i} className="flex items-center gap-3 cursor-pointer group">
                        <input
                          ...
                          onChange={() => setSelectedSpecs(prev => prev.includes(spec) ? prev.filter(s => s !== spec) : [...prev, spec])}
                          className="w-5 h-5 rounded border-gray-300 text-[#8B39E5] focus:ring-[#8B39E5]"
                        />
                        <span className={`text-[14px] ${selectedSpecs.includes(spec) ? "text-[#8B39E5] font-bold" : "text-gray-600 group-hover:text-black"}`}>{spec}</span>
                      </label>
                    ))}
                  </div>
                )}
                  ...
                  <div className="space-y-6 md:space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-10">
                      <div className="space-y-2">
                        <p className="font-bold text-[16px] md:text-[18px]">Occupation</p>
                      ...
                      <div className="space-y-2">
                        <p className="font-bold text-[16px] md:text-[18px]">Speciality</p>
                        <div className="relative">
                    ...
                    <div className="space-y-4">
                      ...
                      {skillsRows.map((row) => (
                        <div key={row.id} className="flex flex-col md:grid md:grid-cols-2 gap-3 md:gap-10 relative group bg-gray-50 md:bg-transparent p-3 md:p-0 rounded-xl">
                          <div className="relative">
                            ...
                            <input
                              ...
                              placeholder="Figma"
                              value={row.skill}
                              onChange={(e) => setSkillsRows(skillsRows.map(r => r.id === row.id ? { ...r, skill: e.target.value } : r))}
                          ...
                          <div className="relative">
                            ...
                            <input
                              ...
                              onChange={(e) => setSkillsRows(skillsRows.map(r => r.id === row.id ? { ...r, experience: e.target.value } : r))}
                            ...
                            {skillsRows.length > 1 && (
                              <IoCloseCircleOutline
                                ...
                                size={22}
                                onClick={() => setSkillsRows(skillsRows.filter(r => r.id !== row.id))}
                              />
                            )}
                          </div>
                        </div>
                      ...
                      <div className="flex flex-wrap gap-3 pt-2">
                        <button
                          onClick={() => setSkillsRows([...skillsRows, { id: Date.now(), skill: "", experience: "", isDefault: false }])}
                ...
                {activeTab === "Preferences" && (
                  <div className="space-y-6 md:space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                      <div className="space-y-4">
                        <p className="font-bold text-[16px] md:text-[18px] text-[#163D5C]">Employment type</p>
                      ...
                      <div className="space-y-4">
                        <p className="font-bold text-[16px] md:text-[18px] text-[#163D5C]">Workplace type</p>
                      ...
                      <div className="space-y-4">
                        <p className="font-bold text-[16px] md:text-[18px] text-[#163D5C]">Salary range</p>
                      ...
                      <div className="grid grid-cols-2 gap-3 md:gap-4">
                        <div className="space-y-4">
                          <p className="font-bold text-[16px] md:text-[18px] text-[#163D5C]">Country</p>
                          <div className="relative">
                        ...
                        <div className="space-y-4">
                          <p className="font-bold text-[16px] md:text-[18px] text-[#163D5C]">City</p>
                          <div className="relative">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 justify-items-center">
          {filteredTalents.map((talent) => (
            ...
            >
              ...
              <div className="my-4">
                <p className="text-gray-400 text-[11px] md:text-[12px] uppercase tracking-wider font-bold">Minimum Salary</p>
        ...
        {filteredTalents.length === 0 && (
          <div className="text-center py-16 md:py-20 bg-white rounded-[30px] border border-dashed border-gray-300">
            <p className="text-gray-400 text-base md:text-lg">No talents found matching your filters.</p>
          </div>
        )}
      </div>
    </div>
